...
// Tetris pieces - matching dark theme (neutral colors)
const TETRIS_PIECES = [
  // I-piece
  { shape: [[1, 1, 1, 1]], color: 'bg-neutral-50' },
  // O-piece
  { shape: [[1, 1], [1, 1]], color: 'bg-neutral-50' },
  // T-piece
  { shape: [[0, 1, 0], [1, 1, 1]], color: 'bg-neutral-50' },
  // L-piece
  { shape: [[1, 0], [1, 0], [1, 1]], color: 'bg-neutral-50' },
  // S-piece
  { shape: [[0, 1, 1], [1, 1, 0]], color: 'bg-neutral-50' },
  // Z-piece
  { shape: [[1, 1, 0], [0, 1, 1]], color: 'bg-neutral-50' },
  // J-piece
  { shape: [[0, 1], [0, 1], [1, 1]], color: 'bg-neutral-50' },
]
...
export default function TetrisLoading({ 
      ...
      if (linesToClear.length > 0) {
        ...
        const newGrid = prevGrid.map((row, rowIndex) => {
          if (linesToClear.includes(rowIndex)) {
            return row.map(cell => ({ ...cell, color: 'bg-neutral-50 animate-pulse opacity-50' }))
          }
          return row
        })
      ...
      <div key={rowIndex} className="flex">
        {row.map((cell, colIndex) => (
          <div
            ...
            className={`${config.cellSize} border border-neutral-800 transition-all duration-100 ${
              cell.filled 
  ...
  return (
    <div className="flex flex-col items-center justify-center">
      <div className="mb-6">
        <div className={`border-2 border-neutral-800 bg-black ${config.padding} transition-colors`}>
          {renderGrid()}
        </div>
      </div>
      ...
      {showLoadingText && (
        <div className="text-center">
          <p className="text-neutral-400 font-medium transition-colors">{loadingText}</p>
        </div>
      )}
    </div>
  )
